...
function CreateChopCard()
    ...
    TriggerServerEvent('qb-scrapyard:server:giveChopCard', vlist2)
end
...
function CreateListEmail()
    if listed then
        ...
        QBCore.Functions.Notify("You already have a list.", "error")
        return
    end
    GenerateVehicleList()
    if CurrentVehicles ~= nil and next(CurrentVehicles) ~= nil then
                ...
                QBCore.Functions.TriggerCallback('qb-scrapyard:checkOwnerVehicle',function(retval)
                    if retval then
                        ...
                        for _, partInfo in ipairs(ChopParts) do
                            if not partInfo.condition or partInfo.condition(vehicle) then
                                threads +=1
                                CreateThread(function()
                                    ...
                                    QBCore.Functions.Progressbar("scrap_part", Lang:t('text.demolish_vehicle'), ScrapTime, false, true, {
                                        ...
                                        TriggerServerEvent("qb-scrapyard:server:getSmallRewards")
                                    end, function() -- cancel
                                        ClearPedTasks(ped)
                        ...
                        TriggerServerEvent("qb-scrapyard:server:getBigRewards")
                        threads = 0
                        ChopListFinished()
                    else
...
CreateThread(function()
	...
	exports['qb-target']:AddTargetEntity(ped, {
		options = {
			{
				type = "server",
				event = "qb-scrapyard:server:sellCarParts",
...
CreateThread(function()
	...
	exports['qb-target']:AddTargetEntity(ped, {
		options = {
			{
				type = "client",
				event = "qb-scrapyard:client:StartListEmail",
				icon = 'fas fa-user-secret',
...
RegisterNetEvent("qb-scrapyard:client:StartListEmail", function()
	CreateListEmail()
end)

RegisterNetEvent("qb-scrapyard:client:checkList")
AddEventHandler("qb-scrapyard:client:checkList", function()
